...
void AAlsCharacterExample::SetupPlayerInputComponent(UInputComponent* Input)
{
	...
	if (IsValid(EnhancedInput))
	{
		...
		EnhancedInput->BindAction(StickMove_Left_Step1_Action,ETriggerEvent::Triggered, this, &ThisClass::Input_StickMovement_Left_Step1);
		EnhancedInput->BindAction(StickMove_Left_Step2_Action,ETriggerEvent::Triggered, this, &ThisClass::Input_StickMovement_Left_Step2);
	}
}
...
{
	const FVector2D Vec = ActionValue.Get<FVector2D>();
	Input_OnMove_Internal(Vec);
...
void AAlsCharacterExample::Input_OnMove_Internal(const FVector2D& MoveVector)
{
	const auto Value{UAlsVector::ClampMagnitude012D(MoveVector)};
